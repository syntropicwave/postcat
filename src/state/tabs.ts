...
/** A tab stripped of transient/response state, for persistence. */
...
export const useTabs = create<TabsState>()(
  persist(
    (set, get) => ({
      ...
      setUrl: (id, url) => {
        const disabled = (
          get().tabs.find((t) => t.id === id)?.params ?? []
        ).filter((p) => !p.enabled);
        get().updateTab(id, {
          url,
    ...
    {
      name: "postcat.workspace",
      version: 1,
      // Persist only the request side of each tab — responses/streams are
      // transient (and live in history anyway).
      partialize: (s) => ({
...
// Ensure a valid active tab: keep the restored one, else fall back to the
// first (fresh launch, or the restored id is no longer present).
useTabs.setState((s) => ({
  activeTabId: s.tabs.some((t) => t.id === s.activeTabId)
    ? s.activeTabId
    : s.tabs[0].id,
}));
